...
``pypbomb`` contains a series of tools that can be used to design or evaluate a first-pass estimate of the operational envelope of closed-end detonation tubes. This package meant to provide a good starting point in the design of a detonation tube, not to replace the analysis process entirely. A healthy safety factor is recommended given the dynamic nature of detonations, as well as a more in-depth analysis of bolted components such as flanges. The first iteration of this package was written during the design of just such a tube, which is currently being used to measure cell sizes of gaseous detonations, and will be used in the near future for the study of detonations in two-phase mixtures.
...
Once the operational limits of a tube are determined, flanges can be sized. `pypbomb.Flange` looks up the minimum necessary flange class based on the maximum tube pressure and temperature based on the standards set forth in ASME B16.5-2003 [@asmeb165]. Again, this analysis does not account for the impulsive loading caused by detonation, and further analysis should be conducted on a per-flange basis.
