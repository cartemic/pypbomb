...
_DIR_LOOKUP_DATA = os.path.join(os.path.dirname(os.path.relpath(__file__)), "lookup_data")
MATERIAL_LIMITS = dict(
    welded=pd.read_csv(
        os.path.join(_DIR_LOOKUP_DATA, "ASME_B31_1_stress_limits_welded.csv"),
        # index_col=0
    ),
    seamless=pd.read_csv(
        os.path.join(_DIR_LOOKUP_DATA, "ASME_B31_1_stress_limits_seamless.csv"),
        # index_col=0
    ),
)
PIPE_SCHEDULES = pd.read_csv(os.path.relpath(os.path.join(_DIR_LOOKUP_DATA, "pipe_schedules.csv")), index_col=0)
...
def _collect_tube_materials():
    ...
    df_materials : pd.DataFrame
        ...
        properties
    """
    file_directory = os.path.join(os.path.dirname(os.path.relpath(__file__)), "lookup_data")
    ...
    with warnings.catch_warnings():
        ...
        # apply units
        df_materials["ElasticModulus"] = [_Q(item, "GPa") for item in df_materials["ElasticModulus"].values]
        df_materials["Density"] = [_Q(item, "g/cm^3") for item in df_materials["Density"].values]
...
def _check_materials_list():
    ...
    if not bool(flange_ratings + stress_limits):
        raise FileNotFoundError("No files containing 'flange' or 'stress' found")
    ...
    for file in stress_limits:
        if ("welded" not in file.lower()) and ("seamless" not in file.lower()):
            ...
            warnings.warn(file + " does not indicate whether it is welded or seamless")

        # check the first row of the file in question to extract the names
        # of the materials that it contains stress limits for
        file_location = os.path.join(file_directory, file)
        with open(file_location, "r") as current_file:
            ...
            for item in TUBE_MATERIALS["Grade"].values.astype(str):
                if item not in materials:
                    ...
                    # error string.
                    error_string += "Material " + item + " not found in " + file_location + "\n"
                    has_errors = True
...
def _check_material(material):
    if material not in TUBE_MATERIALS.Grade.values:
        msg = "%s not found in materials." " Try Flange.available_materials" % material
...
def _get_flange_limits_from_csv():
    ...
    for group in groups:
        ...
        file_directory = os.path.join(os.path.dirname(os.path.relpath(__file__)), "lookup_data")
        file_name = "ASME_B16_5_flange_ratings_group_" + file_group + ".csv"
        file_location = os.path.relpath(os.path.join(file_directory, file_name))
        ...
        new_data = np.array(
            [pd.to_numeric(flange_limits[column].values, errors="coerce") for column in flange_limits.columns]
        ).transpose()
        flange_limits = pd.DataFrame(columns=flange_limits.columns, data=new_data).fillna(0)

        # make sure pressures are positive
        if not all(flange_limits.loc[:, flange_limits.columns != "Temperature"].fillna(0).values.flatten() >= 0):
            raise ValueError("\nPressure less than zero.")

        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            # add units to temperature column
            flange_limits["Temperature"] = [_Q(temp, "degC") for temp in flange_limits["Temperature"]]
...
def _import_thread_specs():
    ...
    """
    file_directory = os.path.join(os.path.dirname(os.path.relpath(__file__)), "lookup_data")
    file_names = ["ANSI_inch_internal_thread.csv", "ANSI_inch_external_thread.csv"]
    file_locations = [os.path.relpath(os.path.join(file_directory, name)) for name in file_names]

    thread_specs = {
        key: pd.read_csv(location, index_col=(0, 1)) for location, key in zip(file_locations, ["internal", "external"])
    }
...
class Bolt:
    ...
    def calculate_safety_factors(
        ...
        max_pressure = units.parse_quant_input(max_pressure, unit_registry)
        window_area = units.parse_quant_input(window_area, unit_registry)
        bolt_max_tensile = units.parse_quant_input(bolt_max_tensile, unit_registry)
        plate_max_tensile = units.parse_quant_input(plate_max_tensile, unit_registry)
        engagement_length = units.parse_quant_input(engagement_length, unit_registry)

        units.check_pint_quantity(max_pressure, "pressure", ensure_positive=True)
        units.check_pint_quantity(window_area, "area", ensure_positive=True)
        units.check_pint_quantity(bolt_max_tensile, "pressure", ensure_positive=True)
        units.check_pint_quantity(plate_max_tensile, "pressure", ensure_positive=True)
        units.check_pint_quantity(engagement_length, "length", ensure_positive=True)
        ...
        screw_area = quant(screw_area.magnitude, screw_area.units.format_babel())
        plate_area = thread["plate area"]
        plate_area = quant(plate_area.magnitude, plate_area.units.format_babel())

        # calculate safety factors
        safety_factors = dict()
        safety_factors["bolt"] = (bolt_max_tensile / (window_force / screw_area)).to("").magnitude
        safety_factors["plate"] = (plate_max_tensile / (window_force / plate_area)).to("").magnitude
    ...
    def calculate_stress_areas(
        ...
        bolt_max_tensile = quant(bolt_max_tensile.magnitude, bolt_max_tensile.units.format_babel())
        plate_max_tensile = quant(plate_max_tensile.magnitude, plate_max_tensile.units.format_babel())
        engagement_length = quant(engagement_length.magnitude, engagement_length.units.format_babel())

        thread = dict()

        # look up thread specs for stress area calculations
        k_n_max = quant(
            THREAD_SPECS["internal"]["minor diameter max"][thread_size][thread_class + "B"],
            "in",
        )
        e_s_min = quant(
            THREAD_SPECS["external"]["pitch diameter min"][thread_size][thread_class + "A"],
            "in",
        )
        e_n_max = quant(
            THREAD_SPECS["internal"]["pitch diameter max"][thread_size][thread_class + "B"],
            "in",
        )
        d_s_min = quant(
            THREAD_SPECS["external"]["major diameter min"][thread_size][thread_class + "A"],
            "in",
        )
        tpi = quant(float(thread_size.split("-")[-1]), "1/in")
        basic_diameter = quant(
            THREAD_SPECS["external"]["basic diameter"][thread_size][thread_class + "A"],
            "in",
        )
        ...
        if bolt_max_tensile < quant(100000, "psi"):
            ...
            screw_area_tensile = np.pi / 4 * (basic_diameter - 0.9742785 / tpi) ** 2
        else:
            # calculate screw tensile area using eq. 2b (p. 1490) in Fasteners
            # section of Machinery's Handbook 26
            screw_area_tensile = np.pi * (e_s_min / 2 - 0.16238 / tpi) ** 2

        # calculate screw shear area using eq. 5 (p. 1491) in Fasteners section
        # of Machinery's Handbook 26
        screw_area_shear = np.pi * tpi * engagement_length * k_n_max * (1.0 / (2 * tpi) + 0.57735 * (e_s_min - k_n_max))
        ...
        # of Machinery's Handbook 26
        thread["plate area"] = (
            np.pi * tpi * engagement_length * d_s_min * (1.0 / (2 * tpi) + 0.57735 * (d_s_min - e_n_max))
        )

        # calculate minimum engagement scale factor using eq. 3 (p. 1490) in
        # Fasteners section of Machinery's Handbook 26
        j_factor = (screw_area_shear * bolt_max_tensile) / (thread["plate area"] * plate_max_tensile)
        ...
        thread["minimum engagement"] = (
            2 * screw_area_tensile / (k_n_max * np.pi * (1.0 / 2 + 0.57735 * tpi * (e_s_min - k_n_max)))
        ) * j_factor

        return thread

    @staticmethod
    def get_thread_property(thread_property, thread_size, thread_class, unit_registry):
        """
        ...
        elif "B" in thread_class and ("2" in thread_class or "3" in thread_class):
        ...
        if thread_size not in thread_specs.index:
            raise KeyError("\nThread size '" + thread_size + "' not found")
...
class DDT:
    ...
    @staticmethod
    def calculate_blockage_diameter(pipe_id, blockage_ratio, unit_registry=_U):
        """
        ...
        units.check_pint_quantity(pipe_id, "length", ensure_positive=True)
    ...
    @staticmethod
    def calculate_blockage_ratio(tube_inner_diameter, blockage_diameter, unit_registry=_U):
        """
        ...
        tube_inner_diameter = units.parse_quant_input(tube_inner_diameter, unit_registry).to_base_units()
        blockage_diameter = units.parse_quant_input(blockage_diameter, unit_registry).to_base_units()
        units.check_pint_quantity(tube_inner_diameter, "length", ensure_positive=True)
        units.check_pint_quantity(blockage_diameter, "length", ensure_positive=True)
        ...
        blockage_ratio = 1 - (1 - 2 * blockage_diameter.magnitude / tube_inner_diameter.magnitude) ** 2
    ...
    def calculate_run_up(
        ...
        tube_diameter = units.parse_quant_input(tube_diameter, unit_registry)
        ...
        units.check_pint_quantity(tube_diameter, "length", ensure_positive=True)
        ...
        def eq4_1():
            ...
            rho = units.parse_quant_input((working_gas.density_mass, "kg/m^3"), unit_registry)
            mu = units.parse_quant_input((working_gas.viscosity, "Pa*s"), unit_registry)
            nu = mu / rho
            delta = (nu / laminar_fs).to_base_units()

            # calculate gamma
            gamma = (
                sound_speed / (eta * (density_ratio - 1) ** 2 * laminar_fs) * (delta / tube_diameter) ** (1.0 / 3)
            ) ** (1 / (2 * mm + 7.0 / 3))

            # calculate runup distance
            d_over_h = 2.0 / (1 - np.sqrt(1 - blockage_ratio))
        ...
        def eq4_4():
            ...
            lhs = 2 * 10 * laminar_fs * (density_ratio - 1) / (sound_speed * tube_diameter)
            rhs = aa * (1 - blockage_ratio) / (1 + bb * blockage_ratio)
        ...
        if 0.3 <= blockage_ratio <= 0.75:
            ...
            interp_distances = np.array([eq4_1().magnitude, eq4_4().magnitude])
            runup_distance = np.interp(blockage_ratio, np.array([0.1, 0.3]), interp_distances)
            runup_distance = units.parse_quant_input(
                (runup_distance, tube_diameter.units.format_babel()), unit_registry
            )
...
class Window:
    ...
    @classmethod
    def safety_factor(cls, length, width, thickness, pressure, rupture_modulus, unit_registry=_U):
        """
        ...
        """
        length = units.parse_quant_input(length, unit_registry).to_base_units()
        width = units.parse_quant_input(width, unit_registry).to_base_units()
        thickness = units.parse_quant_input(thickness, unit_registry).to_base_units()
        pressure = units.parse_quant_input(pressure, unit_registry).to_base_units()
        rupture_modulus = units.parse_quant_input(rupture_modulus, unit_registry).to_base_units()

        units.check_pint_quantity(length, "length", ensure_positive=True)
        units.check_pint_quantity(width, "length", ensure_positive=True)
        units.check_pint_quantity(thickness, "length", ensure_positive=True)

        units.check_pint_quantity(pressure, "pressure", ensure_positive=True)
        units.check_pint_quantity(rupture_modulus, "pressure", ensure_positive=True)

        # https://www.crystran.co.uk/userfiles/files/design-of-pressure-windows.pdf
        k = 0.75
        r = length / width
        safety_factor = (
            ((thickness / length) ** 2 * (2 * rupture_modulus * (1 + r**2)) / (k * pressure)).to("").magnitude
        )
        return safety_factor

    @classmethod
    def minimum_thickness(cls, length, width, safety_factor, pressure, rupture_modulus, unit_registry=_U):
        """
        ...
        k = 0.75  # clamped
        r = length / width
        thickness = (length * np.sqrt(safety_factor * k / 2) * np.sqrt(pressure / (rupture_modulus * (1 + r**2)))).to(
            width.units.format_babel()
        )
...
class Tube:
    ...
    def calculate_max_stress(cls, initial_temperature, material, welded, unit_registry=thermochem._U):
        ...
        stress_limits = cls._get_pipe_stress_limits(material=material, welded=welded, unit_registry=unit_registry)
        stress_units = stress_limits.values[0].units
        stresses = list(map(lambda x: x.magnitude, stress_limits.values))
        temp_units = stress_limits.index[0].units
        temperatures = list(map(lambda x: x.magnitude, stress_limits.index.values))
        ...
        if not np.all(np.diff(temperatures) > 0):
            raise ValueError("\nStress limits require temperatures to be " + "monotonically increasing")

        # interpolate max stress
        max_stress = unit_registry.Quantity(
            np.interp(initial_temperature.to(temp_units).magnitude, temperatures, stresses),
            stress_units,
        )
    ...
    @staticmethod
    def _get_pipe_stress_limits(material, welded, unit_registry=_U):
        """
    ...
    def dynamic_load_factor(
        ...
        radius = (tube_od + tube_id) / 4.0
        tube_thickness = (tube_od - tube_id) / 2.0
        ...
        bounds = [
            (crit_velocity * (1.0 + plus_or_minus)).to("m/s").magnitude,
            (crit_velocity * (1.0 - plus_or_minus)).to("m/s").magnitude,
        ]
    ...
    @staticmethod
    def calculate_max_pressure(tube_id, tube_od, max_stress, safety_factor=1):
        """
        ...
        """
        mean_diameter = (tube_od + tube_id) / 2.0
        wall_thickness = (tube_od - tube_id) / 2.0
        max_pressure = max_stress * (2 * wall_thickness) / (mean_diameter * safety_factor)
    ...
    def calculate_max_initial_pressure(
        ...
        error = (initial_pressure.magnitude * dlf / max_pressure.magnitude) - 1.0
    ...
    def get_available_pipe_schedules(pipe_size):
        ...
        if pipe_size not in PIPE_SCHEDULES.index:
            msg = "Invalid pipe size: %s. " "See Tube.available_pipe_sizes." % str(pipe_size)
            raise ValueError(msg)
        else:
            return list(PIPE_SCHEDULES.T[pipe_size].dropna().index.drop("OD"))

    @classmethod
    def get_dimensions(cls, pipe_size, pipe_schedule, unit_registry=_U):
        """
        ...
        if pipe_schedule not in cls.get_available_pipe_schedules(pipe_size):
            msg = (
                ...
                "Tube.get_available_pipe_schedules.".format(str(pipe_schedule), str(pipe_size))
            )
            raise ValueError(msg)
    ...
    @classmethod
    def get_elastic_modulus(cls, material, unit_registry=_U):
        """
        ...
        return units.parse_quant_input(
            TUBE_MATERIALS[TUBE_MATERIALS["Grade"] == material]["ElasticModulus"].values[0],
            unit_registry,
        )

    @classmethod
    def get_density(cls, material, unit_registry=_U):
        """
        ...
        cls._check_material(material)
        return units.parse_quant_input(
            TUBE_MATERIALS[TUBE_MATERIALS["Grade"] == material]["Density"].values[0],
            unit_registry,
        )

    @classmethod
    def get_poisson(cls, material):
        """
        ...
        """
        cls._check_material(material)
        return TUBE_MATERIALS[TUBE_MATERIALS["Grade"] == material]["Poisson"].values[0]

    @classmethod
    def _check_material(cls, material):
        """
...
class Flange:
    ...
    @staticmethod
    def get_class(max_pressure, temperature, material, unit_registry=_U):
        """
        ...
        units.check_pint_quantity(max_pressure, "pressure", True)
        units.check_pint_quantity(temperature, "temperature", True)
        max_pressure = units.parse_quant_input(max_pressure, _U)
        temperature = units.parse_quant_input(temperature, _U)

        _check_material(material)
        group = str(TUBE_MATERIALS[TUBE_MATERIALS.Grade == material].Group.values[0])
        ...
        if (temperature < df_limits["Temperature"].min()) or (temperature > df_limits["Temperature"].max()):
            raise ValueError("Temperature out of range.")

        df_limits = df_limits.applymap(lambda x: x.to_base_units().magnitude)
        ...
        return ser_result.keys()[ser_result >= max_pressure.to_base_units().magnitude][0]

    @staticmethod
    def get_max_pressure(flange_class, temperature, material):
        """
        ...
        """
        _check_material(material)
        group = str(TUBE_MATERIALS.Group[TUBE_MATERIALS.Grade == material].iloc[0])
    ...
    @staticmethod
    def get_max_temperature(flange_class, pressure, material):
        """
        ...
        if (pressure < df_limits[flange_class].min()) or (pressure > df_limits[flange_class].max()):
            raise ValueError("Pressure out of range.")

        df_limits = df_limits.applymap(lambda x: x.to_base_units().magnitude)[["Temperature", flange_class]]
